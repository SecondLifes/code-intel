...
|---|---|
| "How do I upload/query points in Qdrant?" / any Python `qdrant-client` question | Use the curated snippet search (`curl skills.qdrant.tech/snippets/search?language=python&query=...`) before writing client code from memory — this repo's own dependency is `qdrant-client` (Python), matching the `python` language filter. |
| Ambiguous/no specific client question | Check the official SDK list below first — this repo only uses the Python client. |
