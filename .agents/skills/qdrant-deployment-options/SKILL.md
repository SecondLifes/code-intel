...
|---|---|
| "Should we move CodeIntel's Qdrant to Cloud/cluster/EDGE?" | Walk the decision tree below against this repo's actual profile: single-operator local-first tool, `mcp-config.json`'s `qdrant_url` defaulting to `http://127.0.0.1:6333` — currently self-hosted/local by design, not a given to change. |
| Ambiguous/no specific deployment question | Read the "Getting Started or Prototyping" section first — closest match to this repo's current setup. |
