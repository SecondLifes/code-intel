...
|---|---|
| "Add a new endpoint for X" / "add a route to search_routes.py" | Consult this skill for the current `Depends`/response-model/streaming patterns before writing the route — keep it thin, delegate logic to `src/services/*_svc.py` per this repo's own convention. |
| "Add SSE/streaming to an endpoint" | Read `references/streaming.md` for the `EventSourceResponse`/`yield` pattern — matches `/api/ask/stream` and `/api/research/stream`'s existing style. |
| "Add a dependency-injected client (Qdrant/Ollama)" | Read `references/dependencies.md` for `Annotated[..., Depends(...)]` — this repo's DIP convention for route handlers. |
| Ambiguous/no specific FastAPI question | Skim `Quick Reference` first; it links to the relevant `references/*.md` file by topic. |
